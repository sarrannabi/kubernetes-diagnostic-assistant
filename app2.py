...
def fetch_stackoverflow():
    # Pour simplifier on ne le fait pas ici, juste un placeholder
...
def ask():
    ...
    try:
        retrieved, diagnosis = query_ai(client, embedder, query)
        return jsonify({"root_cause": diagnosis.split("Root cause:")[-1].split("Fix:")[0].strip(),
                        "fix": diagnosis.split("Fix:")[-1].strip()})
